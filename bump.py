...
def main(fast=True):
    ...
    #
    reviver_skill_bump_plot_data(conn, cursor)
...
def reviver_skill_bump_plot_data(
    conn,
    cursor
):
    cursor.execute(
    ...
    print(chart_data)
    print(json.dumps(chart_data, indent=3))

    with open('reports/faction/revives/json/skills.json', 'w') as f:
